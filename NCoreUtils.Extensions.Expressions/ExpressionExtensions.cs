...
namespace NCoreUtils
{
    ...
    {
        private sealed class ParameterSubstitution : ExtensionExpressionVisitor
        {
        ...
        private sealed class SpliceInliner : ExtensionExpressionVisitor
        {
            ...
            protected override Expression VisitMethodCall(MethodCallExpression node)
            {
                if (node.Method.DeclaringType == typeof(Splice))
                {
                    ...
                    {
                        if (!node.Arguments[0].TryExtractConstant(out var boxedLambda) || boxedLambda is not LambdaExpression lambda)
                        {
