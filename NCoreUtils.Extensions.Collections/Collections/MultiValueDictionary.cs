    ...
    where TKey : notnull
{
    private struct Entry : IEquatable<Entry>
    {
        ...
        public readonly bool Equals(Entry that, IEqualityComparer<TKey> keyComparer, IEqualityComparer<TValue> valueComparer)
            => HashCode == that.HashCode && Next == that.Next && keyComparer.Equals(Key, that.Key) && valueComparer.Equals(Value, that.Value);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public readonly bool Equals(Entry that)
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private int GetKeyHasCode(TKey key)
        => _keyComparer.GetHashCode(key) & 0x7FFFFFFF;
    ...
    private bool FindEntries(TKey key, ICollection<int> entries)
    {
        if (key is null)
        {
    ...
    private int Insert(TKey key, TValue value, int hashCode, int targetBucket0)
    {
    ...
    private int Remove(TKey key, Func<int, bool> predicate)
    {
        if (key is null)
        {
            throw new ArgumentNullException(nameof(key));
    ...
    private bool Contains(TKey key, Func<int, bool> predicate)
    {
        if (key is null)
        {
            throw new ArgumentNullException(nameof(key));
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Set(TKey key, params TValue[] values)
        => Set(key, (IEnumerable<TValue>)values);
    ...
    public override bool Equals(object? obj)
        => Equals(obj as MultiValueDictionary<TKey, TValue>);
